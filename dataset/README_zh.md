...
dataset/
├── README.md                          # 本说明（英文版）
├── README_zh.md                       # 本说明（中文版）
├── pre-training/                      # 预训练数据（17 个 SAR 数据集，约 360K 张）
...
检测与分割数据使用 MMDetection/MMRotate/MMSegmentation 的标准格式（COCO、DOTA、自定义 JSON 标注或语义分割标签图），具体结构见 `detection/mmrotate/` 与 `segmentation/mmseg/` 下的配置文件说明。

## 常用数据获取渠道

- **MSTAR**：公开经典 SAR 车辆数据集（10 类，12,092 张），可通过官方、镜像渠道获取
