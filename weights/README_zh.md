...
│   ├── pixel/                     # 目标消融：像素重建
│   ├── single_s1 .. single_s6/    # 目标消融：单尺度目标
│   └── multi/                     # 目标消融：融合目标
...
- **线性评测**（`classification/linear_eval`）：`SARATRX_PRETRAIN_CKPT`（默认 `weights/base/jiaquan_simple/checkpoint-1200.pth`）
- **少样本**（`classification/fewshot`）：`SARATRX_PRETRAIN_CKPT`（iTPN）/ `SARATRX_HIVIT_CKPT`（HiViT）
- **检测**（`detection`）：`ITPN_CKPT`（iTPN-B）/ `ITPN_LARGE_CKPT`（iTPN-L），默认 `ckpts/iTPN/...`
- **分割**（`segmentation`）：在 config 中设置 `model.pretrained` 或传入 `--cfg-options model.pretrained=...`
