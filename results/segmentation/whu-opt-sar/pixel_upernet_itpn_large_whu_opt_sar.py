# =============================================================================
# WHU-OPT-SAR semantic segmentation config (reference reproduction, iTPN-Large)
# UperNet + iTPN-Large (MMSegmentation 1.x / mmengine style)
#
# NOTE / 说明:
# - Archived from the original experiment that produced the paper's
#   WHU-OPT-SAR iTPN-Large numbers (see the sibling training log in this folder).
# - Uses the custom `WHUDataset`, `iTPN_weijie` backbone and
...
#
# 此配置为论文 WHU-OPT-SAR（iTPN-Large）结果所用实验配置的存档（训练日志见本目录），
# 依赖自定义 `WHUDataset`、`iTPN_weijie` 骨干与 `itpn_LayerDecayOptimizerConstructor`。
# 下方绝对路径需按环境修改。
# =============================================================================
auto_scale_lr = dict(enable=False, base_batch_size=32)
work_dir = './work_dirs/large_weijie_whu_1xb4_80k_4e-4_dp20_ld90'
model = dict(
    type='EncoderDecoder',
...
data_root = '/root/autodl-tmp/WHU-OPT-SAR/'
crop_size = (512, 512)
train_pipeline = [
...
train_dataloader = dict(
    ...
    dataset=dict(
        type='WHUDataset',
        data_root='/root/autodl-tmp/WHU-OPT-SAR/',
        data_prefix=dict(img_path='SAR', seg_map_path='Label'),
        ann_file='train.txt',
        pipeline=[
            dict(type='LoadImageFromFile'),
            dict(type='LoadAnnotations', reduce_zero_label=True),
