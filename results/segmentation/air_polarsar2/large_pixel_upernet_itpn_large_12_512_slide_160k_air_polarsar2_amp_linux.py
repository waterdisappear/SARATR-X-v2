# =============================================================================
# AIR-PolSAR-Seg-2.0 semantic segmentation config (reference reproduction, iTPN-Large)
# UperNet + iTPN-Large (MMSegmentation style)
#
# NOTE / 说明:
# - Archived from the original experiment that produced the paper's
#   AIR-PolSAR-Seg-2.0 iTPN-Large numbers (see the sibling training log).
...
#
# 此配置为论文 AIR-PolSAR-Seg-2.0（iTPN-Large）结果所用实验配置的存档；
...
# base config reference: /mnt/data7t/lwj/mmseg_lwj/configs/itpn/pixel_upernet_itpn_large_12_512_slide_160k_air_polarsar2_amp_linux.py
_base_ = [
...
model = dict(
    pretrained='/mnt/data7t/lwj/mmseg_lwj/ckpt/large/checkpoint-1200.pth',
    backbone=dict(
        type='iTPN',
        img_size=512,
        patch_size=16,
...
optimizer = dict(
    ...
    paramwise_cfg=dict(num_layers=44, layer_decay_rate=0.95))
