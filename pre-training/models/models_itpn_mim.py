...
将 iTPN 分层编码器接入 MAE 掩码重建流程：
  * 编码器输出多尺度 latent（FPN，num_outs=3），各层级投影到共享 decoder 空间；
  * 在掩码位置插入 mask token 并恢复原始顺序，加性融合后经 Transformer decoder
    重建被掩码 patch 的目标（由 MaskedAutoencoder.forward_loss 决定目标空间）。
工厂函数：
  itpn_base_dec512d8b : base 版（embed 512 / 24 层），论文主模型
...
class iTPNMaskedAutoencoder(MaskedAutoencoder, iTPN):
    def __init__(self, img_size=224, patch_size=16, in_chans=3, num_classes=1000, fpn_dim=256, fpn_depth=2,
        ...
        self.decoder_pred = nn.Linear(decoder_embed_dim, 256,
...
def itpn_large_dec512d8b(**kwargs):
    model = iTPNMaskedAutoencoder(
        ...
        num_outs=3, decoder_embed_dim=768, decoder_depth=8, decoder_num_heads=16,
        rpe=False, norm_layer=partial(nn.LayerNorm, eps=1e-6), **kwargs)
    return model
