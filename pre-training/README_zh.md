...
|---|---|
| `pixel` | 原始像素重建（基线） |
| `single_s1` | S1: 3×3 盲点邻域聚合（中心像素置 0） |
...
torchrun --nproc_per_node=8 main_pretrain.py \
    ...
    --init_ckpt ../weights/base/itpn/itpn_base_fpn256.pth   # 可选 warm-start
```
...
```
pre-training/
├── main_pretrain.py                  # 预训练主入口
├── main_finetune.py                  # 全监督微调入口
...
│   ├── models_itpn_mim.py            # iTPN 掩码自编码器
│   └── __init__.py
└── util/
    ├── datasets.py                   # 数据加载
