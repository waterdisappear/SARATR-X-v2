# AIR-PolSAR-Seg-2.0：验证/测试指标与论文常用项对齐说明
# - PA / OA：整体像素精度（Pixel Accuracy = Overall Accuracy），mmseg Summary 中为 aAcc
# - mIoU：mmseg 标准 mean IoU
# - mPA：各类别像素精度再平均，与 mmseg 汇总表 mAcc 一致（勿与 PA/OA 混淆）
# - Kappa：多类 Kappa；训练见 ValTest hook，独立测试见 tools/test.py（--eval mIoU 后打印）
# （定义见 mmcv_custom/rs_metrics.py）
_base_ = [
