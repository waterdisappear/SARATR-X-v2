...
| WHU-OPT-SAR | `data/whu_opt_sar_mmseg_256_split` | 多模态分割（256 切片） |
| ADE20K | `data/ade/ADEChallengeData2016` | 通用分割参考（官方模型迁移） |

数据转换工具（`tools/`）：

```bash
# 将 AIR-PolSAR-Seg RGB 标签转为 mmseg 索引标签
python tools/convert_air_polarsar_labels.py --train-dir <train> --test-dir <test>

# 按场景将 WHU-OPT-SAR 大图切分为 256 切片
python tools/split_whu_opt_sar_for_mmseg.py --data-root <root>

# 其他辅助脚本：stats_earthmap_label_pixels.py、verify_hivit_pretrain.py、
# visualize_polsar_amplitude_params.py（极化幅度伪 RGB 调试）
```
...
```bash
# 训练（8 卡示例，AIR-PolarSAR-Seg-2.0 幅度版）
...
- **DDHR-SK 分割**：UperNet + iTPN — 参考配置与训练日志见 `results/segmentation/ddhr-sk/`
- **对比实验**：UperNet + HiViT（`configs/hivit/`）、CLIP-UperNet + iTPN（`clip_upernet_itpn_*`）

具体数值以论文实验章节为准；各数据集日志与配置（含 WHU-OPT-SAR 与 DDHR-SK）见 `results/segmentation/`。
