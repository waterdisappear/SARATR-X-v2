...
def load_checkpoint(model,
    ...
    if "rel_pos_bias.relative_position_bias_table" in state_dict:
        ...
        for i in range(model.mlp_depth1+model.mlp_depth2+2, num_layers+2):
            state_dict["blocks.%d.attn.relative_position_bias_table" % i] = rel_pos_bias.clone()
            print("blocks.%d.attn.relative_position_bias_table" % i)
    ...
    if 'absolute_pos_embed' in state_dict:
        pos_embed_checkpoint = state_dict['absolute_pos_embed']
        ...
        if orig_size != new_size:
            ...
            state_dict['absolute_pos_embed'] = new_pos_embed
    '''
        ...
        if nH1 != nH2:
            ...
            if L1 != L2:
                ...
                table_pretrained_resized = F.interpolate(
                     ...
                     size=(S2, S2), mode='bicubic', align_corners=False)
...
def load_checkpoint_hivit(model,
                          ...
                          strict=False,
                          logger=None):
    """Load HiViT pretrain checkpoint (supports ``model`` / ``state_dict`` keys)."""
    checkpoint = _load_checkpoint(filename, map_location)
    if not isinstance(checkpoint, dict):
        raise RuntimeError(
            f'No state_dict found in checkpoint file {filename}')
    ...
    for key in all_keys:
        if "relative_position_bias_table" not in key or key not in state_dict:
            continue
        rel_pos_bias = state_dict[key]
        src_num_pos, num_attn_heads = rel_pos_bias.size()
        dst_num_pos, _ = model.state_dict()[key].size()
        ...
        dst_size = int((dst_num_pos - num_extra_tokens) ** 0.5)
        if src_size != dst_size:
            msg = ("Position interpolate for %s from %dx%d to %dx%d" % (
                key, src_size, src_size, dst_size, dst_size))
