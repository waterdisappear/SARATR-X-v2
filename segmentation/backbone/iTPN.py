"""
MMSegmentation 版 iTPN 骨干 (iTPN backbone for MMSegmentation).

与 `pre-training/models/models_itpn.py` 中的 iTPN 编码器同源，但面向语义分割：
- 用 mmseg 的 BACKBONES 注册表注册；
- 输出多尺度特征（num_outs），供 UperNet / CLIP-UperNet 等分割头使用；
- 通过 mmcv_custom.load_checkpoint 加载 MIM 预训练权重（支持去掉前缀、插值位置编码）。

(Share the same iTPN encoder as pre-training, adapted for segmentation heads.)
"""
import math
import torch
...
class iTPN(nn.Module):
    ...
    def interpolate_pos_encoding(self, x, h, w):
        ...
        dim = x.shape[-1]
        # 使用 size 而非 scale_factor，避免 PyTorch 对 bicubic / recompute_scale_factor 的重复 UserWarning
        ...
        assert h == patch_pos_embed.shape[-2] and w == patch_pos_embed.shape[-1]
    ...
    def forward_features(self, x, ids_keep=None, mask=None):
        ...
        if self.num_outs > 3:
            ...
            # outs[0] = torch.cat([outs[0], x_], dim=-1)
