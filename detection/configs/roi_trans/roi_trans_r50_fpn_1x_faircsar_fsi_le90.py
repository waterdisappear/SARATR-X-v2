# RoI Transformer (R50-FPN, le90) on FAIR-CSAR FSI
# 检测头与 train/test 超参与官方 `roi_trans_r50_fpn_1x_dota_le90.py` 一致，仅替换数据与 SAR 归一化。
#
# 与论文表格严格对齐需自行核对：划分（是否全 trainval）、训练轮数（1x=12 epoch）、
# batch 与 lr 线性缩放、是否多尺度测试 / 集成等；本配置与 MMRotate DOTA 版 RoI Trans 一致。
#
# 推荐全局 batch≈4（与常见 2×2 GPU 设置一致）：四卡时用 samples_per_gpu=1，lr=0.005；
# 若四卡且 samples_per_gpu=2（全局 batch=8），请将 lr 改为 0.01（线性缩放）。
#
# 离线骨干：默认加载 ckpts/resnet50-0676ba61.pth（勿用 torchvision://）
# 有网时下载：wget -O ckpts/resnet50-0676ba61.pth \
#   https://download.pytorch.org/models/resnet50-0676ba61.pth
# 其它路径：export RESNET50_CKPT=/path/to/resnet50-0676ba61.pth
