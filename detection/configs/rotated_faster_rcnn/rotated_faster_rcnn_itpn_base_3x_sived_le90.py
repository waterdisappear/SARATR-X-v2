# Rotated Faster R-CNN + iTPN_pixel（base）on SIVED，3x（36 epoch）
#
# 参考 Rotated_fast_itpn：`configs/rotated_faster_rcnn/rotated-faster-rcnn-le90_itpn_base_fpn256_1x_dota.py`
# （mmdet.FasterRCNN + iTPN + neck=None + 五层 stride）。
# 本仓库训练入口为 MMRotate 0.3，故映射为 `RotatedFasterRCNN` + 同名骨干/无 neck，数据与
# `configs/redet/redet_itpn_base_3x_sived_le90.py` 一致。
