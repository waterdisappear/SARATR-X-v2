# ReDet + iTPN_pixel on FAIR-CSAR（SL-TRAINVAL / SL-TEST）
# 以 `redet_itpn_base_3x_rsar.py` 为模板，数据与 `oriented_rcnn_itpn_base_3x_faircsar_sl.py` 对齐
...
# 有效 batch ≈ samples_per_gpu * num_gpus；若 OOM 可改回 1
data = dict(
    samples_per_gpu=2,
    workers_per_gpu=4,
    train=dict(
...
# 原 batch=1×4=4、lr=8e-5；现 batch=2×4=8，按 sqrt 缩放约 1.41，取 1.1e-4 略保守
optimizer = dict(
    _delete_=True,
    type='AdamW',
    lr=1.1e-4,
...
# min_lr_ratio 提高：末期仍保持一定步长，避免 cosine 底过小「白训」
lr_config = dict(
    _delete_=True,
...
# SWA：priority 须 < EvalHook(50)；SWARestoreHook 在评测后恢复训练权重
custom_hooks = [
    dict(
        type='SWAHook',
        swa_start=24,
