# ReDet + iTPN_pixel **Large** on FAIR-CSAR（SL-TRAINVAL / SL-TEST）
# 数据与 `redet_itpn_base_3x_faircsar_sl.py` 一致；骨干参数对齐
# `pixel_itpn_GFL_hrsid_jpg_large.py`（embed_dim=768, depth=40, mlp_depth1/2=2 等）
...
# 预训练权重（iTPN-Large）：环境变量 ITPN_LARGE_CKPT 优先，默认 ckpts/iTPN_large/checkpoint-1200.pth
pretrained = os.environ.get('ITPN_LARGE_CKPT', 'ckpts/iTPN_large/checkpoint-1200.pth')
...
# Large 显存占用更高；若 OOM 将 samples_per_gpu 改为 1
data = dict(
    samples_per_gpu=2,
    workers_per_gpu=4,
    train=dict(
...
# LayerDecay 与 large 参考一致（mlp_depth1+mlp_depth2+depth=44）
optimizer = dict(
    _delete_=True,
    type='AdamW',
    lr=1.1e-4,
