...
- 自定义优化器：`LayerDecayOptimizerConstructor`（含 HiViT 层衰减版）
...
detection/
├── README.md                     # 本文件
├── configs/                      # 论文相关的训练/评测配置（iTPN + SAR 数据集）
...
│   └── _base_/                   # 数据集 / 学习率调度 / 运行时配置
└── mmrotate/                     # 自定义模块（复制到 mmrotate 安装目录覆盖）
...
# 2. 安装 PyTorch（CUDA 11.3 示例）
pip install torch==1.11.0+cu113 torchvision==0.12.0+cu113 \
    --extra-index-url https://download.pytorch.org/whl/cu113

# 3. 安装 mmcv（建议编译安装，避免部分 BUG）
...
> 注：本仓库配置使用 `EpochBasedRunner` / `evaluation` / `custom_hooks` 的 **MMRotate 0.3.x 风格 API**，
> 若使用 mmrotate 1.x（MMEngine Runner），需将 `runner`/`evaluation`/`optimizer_config` 替换为
> `train_cfg`/`val_cfg`/`optim_wrapper` 等新写法。请以实际安装版本为准。
...
| FAIR-CSAR SL | `data/faircsar/SL-TRAINVAL/...` | `FAIRCSAR_SL_TRAIN` / `FAIRCSAR_SL_TEST` | 单视（single-look） |
| FAIR-CSAR FSI | `data/faircsar/FSI-TRAINVAL/...` | `FAIRCSAR_FSI_TRAIN` / `FAIRCSAR_FSI_TEST` | 多视（full-single-look/fused） |
...
| ResNet-50（R50 基线） | `ckpts/resnet50-0676ba61.pth` | `RESNET50_CKPT` |
| fast_iTPN（DOTA 配置） | `ckpts/iTPN/fast_itpn_*.pt` | — |
