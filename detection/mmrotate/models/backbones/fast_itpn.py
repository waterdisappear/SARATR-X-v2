...
def _cfg(url='', **kwargs):
    return {
        'url': url,
        'num_classes': 15, 'input_size': (3, 224, 224), 'pool_size': None,
...
class PatchEmbed(nn.Module):
    ...
    def forward(self, x):
        breakpoint()
        B, C, H, W = x.shape
        patches_resolution = (H // self.patch_size[0], W // self.patch_size[1]) # (64, 64)
        num_patches = patches_resolution[0] * patches_resolution[1] # 4096
        x = self.proj(x).view(      # self.proj(x).shape=torch.Size([2, 128, 256, 256])
            B, -1,
            patches_resolution[0], self.inner_patches,
            patches_resolution[1], self.inner_patches,
        ).permute(0, 2, 4, 3, 5, 1).reshape(B, num_patches, self.inner_patches, self.inner_patches, -1) # torch.Size([2, 4096, 4, 4, 128])
...
class Fast_iTPN(nn.Module):
    def __init__(self, img_size=224, patch_size=16, in_chans=3, embed_dim=512, depth_stage1=3, depth_stage2=3, depth=24,
                 ...
                 attn_drop_rate=0., drop_path_rate=0.0, init_values=0.1, attn_head_dim=None, norm_layer={'type': 'LN', 'eps': 1e-06},
        ...
        if isinstance(norm_layer, dict) and norm_layer.get('type') == 'LN':
            # 使用 partial 创建 LayerNorm，并设置 eps
            norm_layer = partial(nn.LayerNorm, eps=norm_layer.get('eps', 1e-6))
        else:
            # 如果 norm_layer 不是 LN，抛出错误
            raise ValueError("norm_layer must be a dict with type 'LN'")
    ...
    def forward(self, x):
        x = self.forward_features(x)    # torch.Size([4, 3, 224, 224]) -> torch.Size([4, 384])
        x = self.head(x)
        return x
...
class MMDET_Fast_iTPN(Fast_iTPN):
    def __init__(self, out_indices=(0, 1, 2, 3), pretrained=None, **kwargs):
        """
        Args:
            out_indices (tuple): 指定输出的层次索引，例如 (0, 1, 2, 3) 表示输出 4 个尺度的特征图。
            pretrained (str): 预训练权重路径。
            **kwargs: 其他传递给 Fast_iTPN 的参数。
        """
        super().__init__(**kwargs)  # 初始化 Fast_iTPN
        ...
        for i in range(len(out_indices)-1):
            layer = LayerNorm2d(self.embed_dim//(2**(2-i)), eps=1e-6)
            layer_name = f'outnorm{i}'
            self.add_module(layer_name, layer)
            self.out_norms.append(layer)
        fpn4 = nn.Sequential(
                nn.MaxPool2d(kernel_size=2, stride=2),
                LayerNorm2d(self.embed_dim, eps=1e-6),
            )
        self.out_norms.append(fpn4)
        # 加载预训练权重
    ...
    def load_pretrained(self, ckpt=""):
        ...
        pretrained_dict = _ckpt.get('module', _ckpt)  # 兼容不同保存方式
        
        # 过滤不兼容的键
        filtered_dict = {
            k: v for k, v in pretrained_dict.items()
            if not any(key in k for key in ["pos_embed"])
        }
        
        # 加载过滤后的权重
        # incompatible = self.load_state_dict(filtered_dict, strict=False)
        incompatible = self.load_state_dict(filtered_dict, strict=False)
        ...
        print(f"Successfully loaded ckpt {ckpt}")
    ...
    def forward_features(self, x):
        ...
        for i, blk in enumerate(self.blocks[:-self.num_main_blocks]):
            ...
            if i in self.out_indices:
                # # 对输出进行归一化
                # norm_layer = self.out_norms[self.out_indices.index(i)]
                # out = norm_layer(x.permute(0, 2, 3, 1))  # (B, H, W, C)
        ...
        # x = self.norm(x)
        for i in range(self.out_indices.count(-1)):
            outs.append(x.transpose(1, 2).contiguous().view(B, -1, H//16, W//16))
        for i in range(len(outs)):
                outs[i] = self.out_norms[i](outs[i])
        # if self.fc_norm is not None:
        #     if self.cls_token is not None:
        #         x = x[:, 1:, :]
        #     return self.fc_norm(x.mean(1))
