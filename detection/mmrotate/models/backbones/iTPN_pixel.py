...
class iTPN_pixel(nn.Module):
    """iTPN pixel 骨干，与官方 iTPN det 一致。
    Ref: https://github.com/sunsmarterjie/iTPN/tree/main/det
    支持官方配置参数 mlp_depth（等价于 mlp_depth1=mlp_depth2=mlp_depth）。
    """
    def __init__(self, img_size=224, patch_size=16, in_chans=3, num_classes=80, embed_dim=512, mlp_depth1=3,
                 ...
                 **kwargs):
        # 兼容官方 iTPN det 配置：mlp_depth 等价于 mlp_depth1=mlp_depth2
        if 'mlp_depth' in kwargs:
            md = kwargs.pop('mlp_depth')
            mlp_depth1, mlp_depth2 = md, md
        if 'out_embed_dim' in kwargs:
            kwargs.pop('out_embed_dim')  # 与 fpn_dim 一致，忽略
        super().__init__()
    ...
    def init_weights(self):
        self.apply(self._init_weights)
        if self.init_cfg is None or 'checkpoint' not in (self.init_cfg or {}):
            return
        pretrained = self.init_cfg['checkpoint']
        # 训练脚本使用的 logger 名称是 'mmrotate'，该 logger 绑定了 work_dir 的 log_file
        # 用 get_root_logger() 可确保写入 work_dirs/.../*.log
        logger = get_root_logger()
        if os.path.isfile(pretrained):
            # strict=False：若 key 不匹配，会通过 logger.warning 打印 missing/unexpected keys
            load_checkpoint(self, pretrained, strict=False, logger=logger)
            # 强制输出到 stdout，便于在训练日志中确认预训练已加载（init_weights 时 logger 可能尚未绑定控制台）
            try:
                ...
                rank = 0
            if rank == 0:
                print(f'[iTPN_pixel] loaded backbone pretrained from: {pretrained}', flush=True)
            logger.info(f'[iTPN_pixel] loaded backbone pretrained from: {pretrained}')
        else:
