...
# --------------------------------------------------------
"""
iTPN 骨干网络（线性评测 / KNN 评测用）。

与 pre-training/models/models_itpn.py 结构一致，区别仅在于：
  1. 本文件内置 ``itpn_base`` / ``itpn_large`` 工厂函数，直接加载预训练权重；
  2. 加载权重时做了“线性探测”适配（head 前插入 BatchNorm1d、冻结骨干），
     与 pre-training 阶段使用的 iTPN 骨干（无分类头冻结逻辑）不同。
"""
...
# iTPN 预训练权重路径解析：
#   优先使用环境变量 SARATRX_PRETRAIN_CKPT；
#   否则默认指向仓库 weights/ 目录下的主模型权重
#   （weights/jiaquan_simple/checkpoint-1200.pth，见仓库 weights/README.md）。
...
class iTPN(nn.Module):
    def __init__(self, img_size=224, patch_size=16, in_chans=3, num_classes=1000, embed_dim=512, mlp_depth=3, depth=24,
        ...
        # 分类头：单尺度用 backbone 维度；多尺度 FPN 时对各尺度 GAP 后沿通道拼接，再 norm + linear
        if self.num_outs == 1:
            self.fuse_dim = self.num_features
        else:
            self.fuse_dim = self.num_outs * self.fpn_dim
        self.fc_norm = norm_layer(self.fuse_dim)
        self.head = nn.Linear(self.fuse_dim, num_classes) if num_classes > 0 else nn.Identity()
    ...
    def forward(self, x):
        ...
        if self.num_outs == 1:
            ...
            pooled = []
            for out in x:
                pooled.append(out.reshape(out.shape[0], -1, out.shape[-1]).mean(dim=1))
...
def load_itpn_pretrain(model, ckpt_path):
    """加载 iTPN 预训练权重：去掉 module. 前缀、丢弃不匹配的分类头、插值位置编码。"""
...
def itpn_base(NUM, **kwargs):
    """iTPN-Base（embed 512 / 24 层），用于线性探测。"""
    model = iTPN(num_classes=NUM,
...
def itpn_large(NUM, **kwargs):
    """iTPN-Large（embed 768 / 40 层），用于线性探测。"""
    model = iTPN(num_classes=NUM,
                 embed_dim=768, mlp_depth=2, depth=40, num_heads=12, bridge_mlp_ratio=3., mlp_ratio=4.,
                 rpe=False, num_outs=1, norm_layer=partial(nn.LayerNorm, eps=1e-6), **kwargs)
    load_itpn_pretrain(model, resolve_pretrain_ckpt())
