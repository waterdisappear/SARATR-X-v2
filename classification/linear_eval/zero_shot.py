"""
冻结特征 k-NN 评测（Zero-shot k-NN evaluation）

论文中 SAR-VSA / FUSAR-Ship 使用冻结 backbone 的 k-NN 分类评测。
本脚本支持：
  - k-NN 模式（默认）：冻结 backbone 特征，faiss k-NN 分类
    (k-NN mode (default): frozen backbone features classified with faiss k-NN)
  - finetune 模式：在线性分类头（fc_norm + head）上做少量微调
    (finetune mode: train a linear head on top of the frozen backbone)

用法 (Usage)::

    # k-NN 评测（SAR-VSA / FUSAR-Ship 等）
    python zero_shot.py --data_path /path/to/data --mode knn --backbone base \\
        --n_shot 0 --k_values 1,5,10,20

    # 或使用仓库默认数据集路径（dataset/classification/<DATASET>）
    python zero_shot.py --mode knn --backbone base
"""
...
def parameter_setting():
    parser = argparse.ArgumentParser(description='iTPN k-NN / linear evaluation')
    default_data = os.path.join(REPO_ROOT, 'dataset', 'classification') + os.sep
    parser.add_argument('--data_path', type=str, default=default_data,
                        help='data root containing Train/ and Val|test/ folders')
...
def main():
    ...
    if arg.mode == 'knn':
        ...
        print("=" * 50)
        print(f"Zero-shot k-NN Classification (backbone={arg.backbone})")
        ...
        # 初始化分类头并加 BN（affine=False），冻结骨干
        # Initialize the head and prepend BN (affine=False); freeze the backbone
