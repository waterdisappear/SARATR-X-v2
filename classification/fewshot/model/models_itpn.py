...
class BlindSpotConv2d(nn.Module):
    ...
    def __init__(self, in_channels, out_channels, kernel_size=3, stride=1, padding=1):
        ...
        self.register_buffer('blind_mask', self._create_blind_mask())

        self._initialize_weights()

    def _create_blind_mask(self):
    ...
    def _initialize_weights(self):
        ...
        center = self.kernel_size // 2
        self.conv.weight.data[:, :, center, center] = 0

    def forward(self, x):
        x = F.pad(x, pad=[self.padding, self.padding, self.padding, self.padding], mode="reflect")
        masked_weight = self.conv.weight * self.blind_mask
...
class PatchEmbed(nn.Module):
    def __init__(self, img_size=224, patch_size=16, inner_patches=4, in_chans=3, embed_dim=96, norm_layer=None):
        ...
        # self.proj2 = BlindSpotConv2d(in_chans, in_chans, kernel_size=3, stride=1, padding=1)
