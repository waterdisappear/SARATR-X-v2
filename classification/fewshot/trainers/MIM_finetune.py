"""
HiViT 少样本微调（few-shot fine-tuning with HiViT）

基于 Dassl 框架：加载预训练 HiViT 骨干后进行全参数微调（few-shot fine-tuning），
与 MIM_linear 的线性探测形成对比。

Based on the Dassl framework: fine-tune a pretrained HiViT backbone end-to-end
on few-shot data, as opposed to the linear-probing protocol in MIM_linear.
"""
...
class CustomCLIP(nn.Module):
    ...
    def __init__(self, cfg, classnames):
        ...
        # 手动初始化 fc 层（MoCo v3 风格，微调用较小的 std）
        # (manually initialize the fc layer, MoCo v3 style, smaller std for fine-tuning)
        from timm.models.layers import trunc_normal_
        trunc_normal_(model.head.weight, std=2e-5)

        # head 前加 BN（affine=False）
        # (prepend BN (affine=False) to the head)
        model.head = torch.nn.Sequential(torch.nn.BatchNorm1d(model.head.in_features, affine=False, eps=1e-6),
                                         model.head)

        # 微调：全部参数可训练 (fine-tuning: all parameters trainable)
...
class MIM_finetune(TrainerX):
    """HiViT 少样本微调训练器 (HiViT few-shot fine-tuning trainer)."""
